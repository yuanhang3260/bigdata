...
public abstract class AbstractTableInputFormat<T> extends RichInputFormat<T, HbaseTableInputSplit> {
   ...
   * @see Configuration
   */
  public abstract void configure(org.apache.flink.configuration.Configuration parameters);

  @Override
  public void open(HbaseTableInputSplit split) throws IOException {
    if (table == null) {
  ...
  private void logSplitInfo(String action, HbaseTableInputSplit split) {
  ...
  @Override
  public HbaseTableInputSplit[] createInputSplits(final int minNumSplits) throws IOException {
    if (table == null) {
      throw new IOException("The HBase table has not been opened! " +
          "This needs to be done in configure().");
    }
    ...
    final List<HbaseTableInputSplit> splits = new ArrayList<HbaseTableInputSplit>(minNumSplits);
    for (int i = 0; i < keys.getFirst().length; i++) {
      ...
      if ((scanWithNoLowerBound || isLastRegion || Bytes.compareTo(startRow, endKey) < 0) &&
        ...
        int id = splits.size();
        final HbaseTableInputSplit split =
            new HbaseTableInputSplit(id, hosts, table.getName().toBytes(), splitStart, splitStop);
        splits.add(split);
      }
    }
    for (HbaseTableInputSplit split : splits) {
      logSplitInfo("created", split);
    }
    return splits.toArray(new HbaseTableInputSplit[splits.size()]);
  }
  ...
  @Override
  public InputSplitAssigner getInputSplitAssigner(HbaseTableInputSplit[] inputSplits) {
