...
public class HBaseRowInputFormat extends AbstractTableInputFormat<Row> {
  ...
  public void configure(org.apache.flink.configuration.Configuration parameters) {
    ...
    // prepare output rows
    this.resultRow = new Row(families.length + 1);
    this.familyRows = new Row[families.length];

    this.resultRow.setField(0, "");
    for (int f = 0; f < families.length; f++) {
      this.familyRows[f] = new Row(qualifiers[f].length);
      this.resultRow.setField(f + 1, this.familyRows[f]);
    }
  ...
  protected Row mapResultToOutType(Result res) {
    resultRow.setField(0, new String(res.getRow(), stringCharset));
    for (int f = 0; f < this.families.length; f++) {
      ...
      resultRow.setField(f + 1, familyRow);
    }
    return resultRow;
  }
...
//    String[] fieldNames = new String[famNames.length + 1];
//    fieldNames[0] = "row_key";
//    System.arraycopy(famNames, 0, fieldNames, 1, famNames.length);
//
//    TypeInformation<?>[] typeInfos = new TypeInformation[famNames.length + 1];
...
//      i++;
//    }
//    return new RowTypeInfo(typeInfos, fieldNames);
//  }
